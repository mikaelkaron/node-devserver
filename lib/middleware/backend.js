exports = module.exports = function backend() {
	...
	var options = Array.prototype.map.call(arguments, function (option) {
		[ REGEXP, OPTIONS].forEach(function (property) {
	...
	return function (request, response, next) {
		...
		if(!options.some(function (entry) {
			var matches = XRegExp.exec(url, entry[REGEXP]);

			if (matches) {
				entry = transform(copy(entry), matches);

				var method = entry[PROXY] || "web";
				proxy[method](request, response, entry[OPTIONS]);
